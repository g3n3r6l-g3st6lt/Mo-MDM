...
# ===================== Turn-aware A* (kept name for compatibility) =====================
...
def _build_batched_tile_mesh(name_prefix, positions, color, geom, base_z):
    ...
    # Extrude in mesh data by duplicating verts with offset
    top_offset = Vector((0,0,geom[5]))  # tile_h
    n = len(verts)
    verts.extend([v + top_offset for v in verts])  # same order
...
def build_batched_walls_object(name, blocks, cell_w, cell_h, off_x, off_y, height_min, height_max, uniform_height, wall_color, target_col):
    """
    Create one mesh that contains all wall rectangles as prisms.
    Each block can have its own height (random in min..max) unless uniform_height is True.
    """
    verts = []
    faces = []
    # For stable reproducibility with fixed seed, use Python random global (already seeded upstream).
    for (r,c,h,w) in blocks:
        ...
        # faces (quads): sides + top (omit bottom for speed if you want)
        faces.extend([
            [idx+0, idx+1, idx+5, idx+4],  # +X side
            [idx+1, idx+2, idx+6, idx+5],  # +Y side
            [idx+2, idx+3, idx+7, idx+6],  # -X side
            [idx+3, idx+0, idx+4, idx+7],  # -Y side
            [idx+4, idx+5, idx+6, idx+7],  # top
            # (optional) bottom: [idx+0, idx+1, idx+2, idx+3],
        ])
...
class MMM_OT_Generate(Operator):
    ...
    def execute(self, context):
        ...
        # Build one object "MazeWalls" from all blocks
        ...
        # assign explicit material name (already set inside builder)
            ...
            for (name, solver, params, color) in styles:
                if solver=="DIJK":
                    route = ensure_unique_route(bitmap, start, end, params, existing, rng,
                                                max_tries=p.unique_reroute_tries, thresh=p.unique_jaccard_max, clr=clr)
                else:
                    route = ensure_unique_route_meander(bitmap, start, end, rng, p.experiential_trials,
                                                        ...
                                                        max_tries=p.unique_reroute_tries, clr=clr)
                if not route:
