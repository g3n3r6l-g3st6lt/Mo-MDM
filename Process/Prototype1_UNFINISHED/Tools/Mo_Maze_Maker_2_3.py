bl_info = {
    ...
    "description": "A maze generator that explores parsimony.",
    "category": "Add Mesh",
}
...
# ---- module cache (runtime only, safe across operators) ----
MODULE_CACHE = {}
...
def get_valid_cache(scene, read_only=False):
    """
    Returns a valid cache dict.
    - read_only=True: NEVER writes to scene (safe for UI draw); returns a sanitized copy.
    - read_only=False: fixes and writes back to scene if needed (use in operators).
    """
    data = scene.get("mmm_cache")

    if not isinstance(data, dict):
        if read_only:
            return _default_cache()
        data = _default_cache()
        scene["mmm_cache"] = data
        return data
    ...
    if read_only:
        ...
        for k, v in base.items():
            if k not in safe:
                safe[k] = v
        if not isinstance(safe.get("paths"), list):
            safe["paths"] = []
        return safe
    else:
        fixed = False
...
# ---------- Dijkstra (with optional forbids) ----------
...
class MMM_Props(PropertyGroup):
    # Maze & seed
    rows: IntProperty(name="Rows", default=22, min=5, soft_max=300, description="Maze rows (passage cells).")
    cols: IntProperty(name="Cols", default=34, min=5, soft_max=300, description="Maze cols (passage cells).")
    randomize: BoolProperty(name="Random Seed", default=True, description="ON: random seed per Generate.")
    seed: IntProperty(name="Seed", default=12345, min=0, description="Used when Random Seed is OFF.")

    # Geometry & floor
    cell_w: FloatProperty(name="Cell W", default=2.5, min=0.05, description="World X size of one grid cell.")
    ...
    # Style toggles + per-style colors
    use_purist: BoolProperty(name="Purist", default=True, description="Shortest path by steps.")
    ...
    use_avenue: BoolProperty(name="Avenue", default=True, description="Prefer open corridors (avoid tight alleys).")
    col_avenue: FloatVectorProperty(name="Avenue Color", subtype='COLOR', size=4, default=(0.3,1.0,0.5,1))

    use_experiential: BoolProperty(name="Experiential", default=True, description="Long simple scenic path.")
    col_experiential: FloatVectorProperty(name="Experiential Color", subtype='COLOR', size=4, default=(1.0,0.85,0.2,1))

    use_zigzag: BoolProperty(name="ZigZag", default=False, description="Maximize turns without repeats.")
    ...
    # Append (UI shows only after first build)
...
class MMM_OT_Generate(Operator):
    ...
    bl_description = "Create maze, cache the exact grid for pathfinding."
    ...
    def execute(self, context):
        ...
        # Dead-end aware loop carver (simple default values)
        ...
        # Cache (paths start as an empty LIST)
        ...
        self.report({'INFO'}, f"Maze {H}x{W} generated & cached. Use 'Build Path Tiles (All Enabled)'.")
        return {'FINISHED'}

class MMM_OT_BuildPaths(Operator):
    bl_idname = "mmm.build_paths"
    bl_label = "Build Path Tiles (All Enabled)"  # <— the button you click to generate tiles
    bl_description = "Compute and draw floor tiles for all enabled styles in one go (distinct routes + intersections)."

    def execute(self, context):
        p=context.scene.mmm_props
        cache = get_cache(context.scene)
        if not cache or not cache.get("bitmap"):
            self.report({'WARNING'}, "Generate the maze first.")
            return {'CANCELLED'}
        ...
        # styles + params + colors (from properties)
        styles=[]
        ...
        if p.use_avenue:      styles.append(("AVENUE","DIJK", dict(step=1.0, turn=0.2, alley=1.0), tuple(p.col_avenue)))
        ...
        # append allowed only if cache already has paths
        ...
        if not isinstance(cache["paths"], list):
            cache["paths"] = []
        existing = list(cache["paths"])

        if not append:
            # replace mode: clear overlays and reset existing paths
            clear_collection(paths_col)
            existing = []
            cache["paths"] = []

        # ensure StartTile exists
        ...
        # redraw overlays: wipe Path_* (keep StartTile) then draw all paths + intersections
        clear_collection(paths_col, name_prefixes=("Path_", "Path_Intersections_"))

        # intersections across ALL paths (multi-cell)
        counts=Counter()
...
class MMM_OT_ClearPaths(Operator):
    ...
    bl_description = "Remove all path tiles and forget path cache (keeps walls/floor)."
    def execute(self, context):
        paths_col = get_collection("maze_paths")
        clear_collection(paths_col)
...
class MMM_PT_Main(Panel):
    ...
    def draw(self, context):
        ...
        # Styles
        box=L.box(); box.label(text="Styles (choose colors & goals)", icon='IPO_BEZIER')
        ...
        row=box.row(align=True); row.prop(p,"use_avenue");      row.prop(p,"col_avenue")
        ...
        box.label(text="Purist=shortest • Smooth=few turns • Avenue=open corridors • Experiential=long scenic • ZigZag=lots of turns")
        ...
        # Append only after first build (read-only cache here)
        ...
        if has_paths:
            ...
            box.label(text="Append adds more paths on top of existing ones. Off = replace.")

        # <<< The button you use to generate path tiles >>>
